...
const AllReports = () => {
  ...
  const {
    ...
    queryFn: () =>
      fetch(`${process.env.REACT_APP_ApiUrl}reports?email=${user?.email}`, {
        headers: {
  ...
  const handleRemoveProduct = () => {
    setLoading(true);
    fetch(
      `${process.env.REACT_APP_ApiUrl}crockeries/${deleteProduct?.reportedId}?email=${user?.email}`,
      {
        method: "DELETE",
        headers: {
          authorization: `bearer ${localStorage.getItem("access-token")}`,
      ...
      .then((result) => {
        if (result.acknowledged) {
          // setLoading(false)
          fetch(
            `${process.env.REACT_APP_ApiUrl}reports/${deleteProduct?._id}?email=${user?.email}`,
            {
              method: "DELETE",
              headers: {
                authorization: `bearer ${localStorage.getItem("access-token")}`,
  ...
  const handleReportDelete = (report) => {
    // setLoading(true)
    fetch(
      `${process.env.REACT_APP_ApiUrl}reports/${report?._id}?email=${user?.email}`,
      {
        method: "DELETE",
        headers: {
          authorization: `bearer ${localStorage.getItem("access-token")}`,
